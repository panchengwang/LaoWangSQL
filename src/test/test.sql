...
begin
    ...
    perform lw_map_add_geometry(map,geom) from county;
    perform lw_map_add_geometry(map,geom) from railway;
    perform lw_map_add_geometry(map,geom) from town;
    
    
    perform lw_map_end(map);
    -- perform lw_map_save_to_file(map,'/tmp/map_' || lw_uuid() || '.png');
    perform lw_map_save_to_file(map,'/tmp/map.png');
